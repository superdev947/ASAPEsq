...
const MyRequest = ({ navigation }) => {
	...
	const loadList = async () => {
		...
		setRefresh(true)
		setData([])
		const response = await fetchs({ url: Root.client_MyRequestLoad, body: { user_id: user.userid, flag } })
		...
		<Container>
			...
			{!loading && data.length > 0 && (
				<FlatList
					...
					renderItem={({ item }) => (
						<View>
							<TouchableOpacity style={[Styles.ROW, Styles.PH20, Styles.MT10, Styles.PB10]} onPress={() => navigation.navigate('inProgress', item)}>
								<View style={[{ width: '20%' }, Styles.PT20]}>
