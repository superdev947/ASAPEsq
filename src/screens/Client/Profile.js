...
import { Container, Content, Item, Input, Form, Toast, Icon } from "native-base"
...
const Profile = ({ navigation }) => {
	...
	const deleteAccountClientApiCall = async () => {
		const response = await fetchs({
			url: Root.client_DeleteAccount_EndURL, body: {
				...
				<View style={{ zIndex: 0, minHeight: 800 }}>
					...
					<Form style={{ marginLeft: 15, marginRight: 15, marginTop: 10 }}>
						...
						<Item style={{ marginTop: 1, marginLeft: 2, borderColor: '#fff' }} >
							...
							<Input value={user.emergency_name} disabled placeholder="Full Name" />
						</Item>
						<Item style={{ marginTop: 1, marginLeft: 2, borderColor: '#fff' }} >
							<Image source={Images.mobile} style={{ height: 35, width: 35 }} />
							<Input value={user.emergency_number} disabled placeholder="Mobile" />
						</Item>
